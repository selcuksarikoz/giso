...
const f = [{ name: "OpenAI", key: "openai", apiUrl: "https://api.openai.com/v1/chat/completions" }, { name: "Gemini", key: "gemini", apiUrl: "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent", modelName: "gemini-1.5-pro-latest" }, { name: "Claude Sonnet", key: "claude", apiUrl: "https://api.anthropic.com/v1/messages" }, { name: "DeepSeek", key: "deepseek", apiUrl: "https://api.deepseek.com/v1/chat/completions", modelName: "deepseek-chat" }], h = o.join(c.homedir(), ".gsqconfig.json");
  ...
  switch (e2) {
    case "openai":
      return await async function(e3, o3, t3) {
        ...
        try {
          const o4 = await fetch(e3, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${s3}` }, body: JSON.stringify({ model: "gpt-3.5-turbo", messages: [{ role: "user", content: t3 }], temperature: 0.7, response_format: { type: "json_object" } }) }), r3 = await o4.json();
          if (console.log("OpenAI API Response:", JSON.stringify(r3, null, 2)), !o4.ok) throw new Error(`API error: ${((_a = r3.error) == null ? void 0 : _a.message) || o4.statusText}`);
    ...
    case "gemini":
      return await async function(e3, o3, t3) {
        ...
        try {
          const e4 = { contents: [{ parts: [{ text: t3 }] }], generationConfig: { response_mime_type: "application/json", temperature: 0.7 }, safetySettings: [{ category: "HARM_CATEGORY_DANGEROUS_CONTENT", threshold: "BLOCK_ONLY_HIGH" }] }, o4 = await fetch(r3, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(e4), timeout: 1e4 });
          if (!o4.ok) {
            const e5 = await o4.json(), t4 = ((_b = e5.error) == null ? void 0 : _b.message) || `HTTP ${o4.status}`;
            throw new Error(`Gemini API error: ${t4}`);
          }
          const s4 = await o4.json();
          if (!s4.candidates || !Array.isArray(s4.candidates) || !((_f = (_e = (_d = (_c = s4.candidates[0]) == null ? void 0 : _c.content) == null ? void 0 : _d.parts) == null ? void 0 : _e[0]) == null ? void 0 : _f.text)) throw console.error("Invalid Gemini response structure:", s4), new Error("Received malformed response from Gemini API");
          const n2 = s4.candidates[0].content.parts[0].text;
          try {
            const e5 = JSON.parse(n2);
            if (!Array.isArray(e5 == null ? void 0 : e5.suggestions)) throw new Error("Suggestions array missing in response");
            return e5.suggestions.map((e6) => ({ type: e6.type || "", message: e6.message || "No message provided", description: e6.description || "" }));
          } catch (e5) {
    ...
    case "claude":
      return await async function(e3, o3, t3) {
        ...
        const s3 = u(o3);
        if (!s3) throw new Error("Failed to decrypt API key");
        const r3 = await fetch(e3, { method: "POST", headers: { "Content-Type": "application/json", "x-api-key": s3, "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model: "claude-3-sonnet-20240229", max_tokens: 1e3, messages: [{ role: "user", content: t3 }], response_format: { type: "json_object" } }) }), n2 = await r3.json();
        try {
    ...
    case "deepseek":
      return await async function(e3, o3, t3) {
        ...
        try {
          const o4 = await fetch(e3, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${s3}` }, body: JSON.stringify({ model: "deepseek-chat", messages: [{ role: "user", content: t3 }], temperature: 0.7, max_tokens: 2e3, response_format: { type: "json_object" } }), timeout: 1e4 });
          if (!o4.ok) {
...
const w = s(t(r(import.meta.url)), "..", "package.json"), S = JSON.parse(e.readFileSync(w, "utf8")), { version: v } = S, x = process.argv.slice(2);
if ((x.includes("--version") || x.includes("-v")) && (console.log(`gsq version ${v}`), process.exit(0)), x.includes("--init")) !async function() {
  try {
    console.log("Select AI providers (comma-separated numbers):"), f.forEach((e2, o3) => {
      console.log(`${o3 + 1}. ${e2.name} (${e2.apiUrl})`);
    });
    const o2 = (await p("> ")).split(",").map((e2) => parseInt(e2.trim()) - 1).filter((e2) => !isNaN(e2) && e2 >= 0 && e2 < f.length);
    0 === o2.length && (console.error("Error: You must select at least one provider."), console.error("Run gsq --init again."), process.exit(1));
    const t2 = { providers: {} };
    for (const e2 of o2) {
      const o3 = f[e2], s2 = await p(`Enter your ${o3.name} API key: `, true);
      t2.providers[o3.key] = { apiKey: d(s2), apiUrl: o3.apiUrl };
    }
    e.writeFileSync(h, JSON.stringify(t2, null, 2)), console.log(`Configuration saved to ${h}`), process.exit(0);
  } catch (e2) {
    console.error("Initialization failed:", e2.message), process.exit(1);
  }
}();
else if (x.includes("--offer")) !async function() {
  try {
    e.existsSync(h) || (console.error("Error: Configuration file not found. Run gsq --init first."), process.exit(1));
    const t2 = JSON.parse(e.readFileSync(h, "utf8"));
    let s2, r2;
    t2.providers && 0 !== Object.keys(t2.providers).length || (console.error("Error: No providers configured. Run gsq --init first."), process.exit(1));
    try {
...
=== ${t3.name} Suggestions ===`)), Array.isArray(i2) ? i2.forEach((e3) => {
    ...
    try {
      ...
      i(`git commit -m "${e2}"`, { stdio: "inherit" }), console.log(n.green.bold("\n✓ Successfully committed with message:")), console.log(n.cyan(`"${m2}"`));
      try {
  ...
  e.existsSync(h) || (console.log("Error: Configuration file not found. Run gsq --init first."), process.exit(1));
  const o2 = JSON.parse(e.readFileSync(h, "utf8"));
  console.log("Configuration:"), console.log(o2);
} else console.log("Usage:"), console.log("  gsq --init      Initialize configuration"), console.log("  gsq --offer     Generate commit message suggestions"), console.log("  gsq --version   Show version"), console.log("  gsq --config    Show configuration"), process.exit(1);
